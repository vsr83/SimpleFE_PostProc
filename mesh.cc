...
Mesh::Mesh(MeshFile *meshfile_in) {
 ...
 //   std::cout << 1e9*x << " " << 1e9*y << std::endl;
//    std::cout << (*node_strlist)[1] << std::endl;
  ...
  for (std::set<int>::iterator it=physset.begin();
       it!=physset.end(); ++it) {
      std::cout << ":" << *it;
  }
